...
async def load_extensions():
    for filename in os.listdir('./commands'):
        if filename.endswith('.py') and filename != 'loader.py':  # исключаем лоадер
            try:
                await bot.load_extension(f'commands.{filename[:-3]}')  # Загружаем команду (без await)
...
async def reload(ctx):
    ...
    for filename in os.listdir('./commands'):
        if filename.endswith('.py') and filename != 'reload.py':  # исключаем лоадер
            try:
                await bot.reload_extension(f'commands.{filename[:-3]}')  # Перезагружаем команду (без await)
